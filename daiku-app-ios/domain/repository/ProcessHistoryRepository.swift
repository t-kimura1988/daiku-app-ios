...
struct ProcessHistoryRepository {
    ...
    func duringProcessList(parameter: DuringProcessListParameter) async throws -> [DuringProcess] {
            var canceller: AnyCancellable?
            let publisher: AnyPublisher<[DuringProcess], ApiError> = try await ApiProvider.provider(service: ProcessHistoryService.duringProcess(parameter))
